...
/**
 * Checks if a path belongs to an ignored directory (e.g. node_modules, venv, bower_components)
 */
...
export function extractProjectContext(filePath: string): { directory: string; project: string } {
  ...
  // Project label uses the immediate parent folder name, e.g. "frontend", "backend", "web"
...
export async function fetchManifests(
  ...
  // Step 2: If fallback or truncated with few entries, probe common subdirectories
  ...
  for (const entry of treeFiles) {
    ...
    // Ignore node_modules, venv, .git, bower_components, etc.
